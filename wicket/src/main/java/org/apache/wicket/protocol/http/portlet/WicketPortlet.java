...
public class WicketPortlet extends GenericPortlet
{
				...
				if (redirectLocation != null)
				{
					...
					if (validWicketUrl)
					{
						if (previousURL == null || previousURL != redirectLocation)
						{
							...
							String value = getHeaderValue(response, responseState, "Content-Security-Policy");
							response.reset();
							responseState.clear();
							responseState.addHeader("Content-Security-Policy", value);
							continue;
						}
						else
						{
	...
	/**
	 * Retrieves the {@code Content-Security-Policy} header value from both the portlet
	 * {@link MimeResponse} and the underlying servlet response.
	 *
	 * <p>
	 * Two sources are checked, in order:
	 * <ol>
	 * <li><b>Portlet response (buffered)</b> — headers accumulated in {@code responseState} during
	 * Wicket processing (intercepted by {@link PortletServletResponseWrapper} before being flushed
	 * to the portlet container). This covers any CSP value set by Wicket itself.</li>
	 * <li><b>Underlying servlet response</b> — the actual {@link HttpServletResponse} managed by
	 * the portlet container (e.g. Liferay 7). This covers CSP values set by a servlet filter
	 * before the portlet processed the request. Because {@code HttpServletResponse.getHeader()} was
	 * introduced in Servlet 3.0 and the project compiles against Servlet 2.3, this call is made
	 * reflectively so that the code compiles against old APIs but works at runtime on modern
	 * containers.</li>
	 * </ol>
	 *
	 * @param response
	 *            the current {@link MimeResponse}
	 * @param responseState
	 *            the buffered response state that holds servlet-level headers before they are
	 *            flushed to the portlet container
	 * @return the {@code Content-Security-Policy} header value, or {@code null} if not present on
	 *         either the portlet or the underlying servlet response
	 */
	protected String getHeaderValue(MimeResponse response, WicketResponseState responseState, String header)
	{
		// 1. Check headers buffered in WicketResponseState (set by Wicket code via
		//    PortletServletResponseWrapper.setHeader / addHeader, not yet flushed to the portlet).
		String value = responseState.getHeader(header);
		if (value != null)
		{
			return value;
		}

		// 2. Attempt to reach the underlying HttpServletResponse set up by the portlet container.
		//    Portlet containers such as Liferay 7 expose the servlet response either by
		//    implementing HttpServletResponse on the same object or by providing a delegate chain
		//    accessible via getResponse() / getHttpServletResponse().
		//
		//    HttpServletResponse.getHeader(String) is a Servlet 3.0 API. This project compiles
		//    against Servlet 2.3, so the call is made reflectively to avoid a compile-time
		//    dependency while still working at runtime on containers that support Servlet 3.0+.
		HttpServletResponse httpResponse = unwrapHttpServletResponse(response);
		if (httpResponse != null)
		{
			value = reflectiveGetHeader(httpResponse, header);
		}
	...
	/**
	 * Attempts to unwrap a {@link HttpServletResponse} from the given portlet response by walking
	 * the delegate chain via reflective calls to {@code getResponse()} and
	 * {@code getHttpServletResponse()}.
	 *
	 * @param portletResponse
	 *            the portlet response to unwrap
	 * @return the underlying {@link HttpServletResponse}, or {@code null} if none can be found
	 */
	private static HttpServletResponse unwrapHttpServletResponse(PortletResponse portletResponse)
	{
		// Walk the delegate chain through both portlet and servlet response wrappers,
		// keeping track of the deepest HttpServletResponse seen. In Liferay 7 the first
		// HttpServletResponse in the chain is often a Liferay wrapper whose getHeader()
		// does not reflect headers set by servlet filters; we need to reach the Tomcat
		// connector response (backed by CoyoteResponse) where those headers actually live.
		HttpServletResponse deepest = null;
		Object candidate = portletResponse;
		Object previous = null;
		while (candidate != null && candidate != previous)
		{
			if (candidate instanceof HttpServletResponse)
			{
				deepest = (HttpServletResponse)candidate;
			}
			previous = candidate;
			candidate = tryGetDelegate(candidate);
		}
		return deepest;
	}

	/**
	 * Reflectively invokes {@code getHttpServletResponse()}, {@code getCoyoteResponse()}, or
	 * {@code getResponse()} on {@code obj} and returns the first non-null result, or {@code null}
	 * if none of the methods exist or all throw. The caller
	 * ({@link #unwrapHttpServletResponse(PortletResponse)}) continues to call this method
	 * iteratively to walk the full wrapper chain down to the deepest response (typically the
	 * Tomcat connector response backed by CoyoteResponse).
	 *
	 * @param obj
	 *            the object to interrogate
	 * @return the delegate object, or {@code null}
	 */
	private static Object tryGetDelegate(Object obj)
	{
		for (String methodName : new String[] { "getHttpServletResponse", "getCoyoteResponse", "getResponse" })
		{
			try
			{
			...
			{
				log.debug("Failed to unwrap servlet response via " + methodName + "()", e);
			}
		}
		return null;
	}
	...
	/**
	 * Calls {@code HttpServletResponse.getHeader(name)} reflectively to avoid a compile-time
	 * dependency on Servlet 3.0 API (the method was added in Servlet 3.0; this project compiles
	 * against Servlet 2.3).
	 *
	 * @param response
	 *            the servlet response on which to invoke {@code getHeader}
	 * @param name
	 *            the header name
	 * @return the header value, or {@code null} if not set or if the runtime does not support the
	 *         method
	 */
	private static String reflectiveGetHeader(HttpServletResponse response, String name)
	{
		try
		{
			return (String)response.getClass()
					.getMethod("getHeader", String.class)
					.invoke(response, name);
		}
		catch (NoSuchMethodException ignored)
		{
			// Servlet container does not support Servlet 3.0 getHeader()
		}
		catch (Exception e)
		{
			log.debug("Failed to read header '" + name + "' from servlet response", e);
		}
		return null;
	}
